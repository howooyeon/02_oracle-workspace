...
SELECT EMP_NAME, DEPT_CODE, TO_CHAR(TO_DATE(TO_NUMBER(SUBSTR(EMP_NO,1,6))), 'YY"년" MM"월" DD"일"') 생년월일,
...
--  주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR (EMP_NO, 1, 6) AS "주민번호 앞자리",  SUBSTR (EMP_NO, 8) AS "주민번호 뒷자리" , (SUBSTR (EMP_NO, 1, 6)+ SUBSTR (EMP_NO, 8)) AS "앞 뒤자리 합"
FROM EMPLOYEE
WHERE EMP_ID = '201';
