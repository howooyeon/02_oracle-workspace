...
FROM EMPLOYEE
JOIN DEPARTMENT USING ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE SUBSTR(EMP_NO, 1, 2) BETWEEN 70 AND 79
AND SUBSTR(EMP_NO, 8, 1) IN (2, 4)
AND SUBSTR(EMP_NAME, 1, 1) IN '전';
...
AND EXTRACT (YEAR FROM SYSDATE) -
                ...
                EXTRACT (YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1, 6), 'RRMMDD')) + 1) FROM EMPLOYEE)
ORDER BY 3;
...
FROM (SELECT *
        FROM EMPLOYEE
        JOIN JOB USING (JOB_CODE)
        JOIN DEPARTMENT USING ON (DEPT_ID = DEPT_CODE)
...
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID) -- LEFT JOIN이 중요
GROUP BY DEPT_TITLE;
