...
-- FROM -> SELECT ROWNUM (이때 순번이 부여됨. 정렬도 하기 전에 이미 순번 부여)
...
WHERE ROWNUM <= 5
ORDER BY SALARY DESC; --3
--> 정상적인 결과가 조회되지 않음!! (정렬되기도 전에 5명이 추려지고 나서 정렬)
...
--ROWNUM 이랑 전체 컬럼 조회하고 싶음 => 별칭 부여하는 방법으로 
...
SELECT /*ROWNUM, */ DEPT_CODE, FLOOR(평균급여) -- 2. 별칭으로 !
FROM (SELECT DEPT_CODE, AVG(SALARY) AS "평균급여" -- 1. 함수 쓰면 별칭 붙여서
        FROM EMPLOYEE
        GROUP BY DEPT_CODE
        ORDER BY 2 DESC)
WHERE ROWNUM <= 3;
...
/*
    * 순위 매기는 함수 ( WINDOW FUNCTION )
    ...
    - DENSE() RANK() OVER(정렬기준) : 동일한 순위가 있다고 해도 그 다음 등수를 무조건 1씩 증가 시킴
                            EX) 공동 1위가 2명이라도 그 다음 순위를 2위 => 1 1 2 
...
FROM EMPLOYEE;
-- 공동 19위 2명 그 뒤의 순위는 20 => 마지막 순위랑 조회된 행수랑 다름
