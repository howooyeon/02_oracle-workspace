...
-- 각 직급별 총 사원수, 보너스를 받는 사원수, 급여합, 평균 급여, 최저급여, 최대급여 별칭지어서 + 직급명 오름차순 정렬
SELECT JOB_CODE, COUNT(*) AS "총 사원수", COUNT(BONUS) AS "보너스를 받는 사원수",
       SUM (SALARY) AS "급여합", ROUND (AVG(SALARY)) AS "평균 급여",
       MIN(SALARY) AS "최저급여", MAX(SALARY) AS "최대급여"
FROM EMPLOYEE --1
GROUP BY JOB_CODE --2
ORDER BY JOB_CODE; -- 1로 대신써도 된다
...
/*
    <SELECT문 실행 순서>
    5. SELECT * | 조회하고자 하는 컬럼 별칭 | 산술식 | 함수식 AS "별칭"
    1. FROM 조회하고자 하는 테이블명
    2. WHERE 조건식(연산자들 가지고 기술)
    3. GROUP BY 그룹 기준으로 삼을 컬럼 | 함수식
    4. HAVING 조건식(그룹함수를 가지고 기술)
    6. ODER BY 컬럼명 | 컬럼순서 | 별칭[ 'ASC' | DESC ] [ NULLS FIRST | NULLS LAST ]
...
/*
    ...
    - UNION     :  OR | 합집합 (두 쿼리문 수행한 결과값 더한 후 중복되는 값은 한 번만 더해지도록)
...
-- 위의 쿼리문 대신 아래처럼 WHERE 절에 OR써도 해결 가능
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
