...
-- 디폴트값을 바꾸다고 해서 이전에 추가된 데이터가 바뀌는 것은 아니다!
...
/*
    ...
    FORIEGN KEY : ADD FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명[(컬럼명)] 
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT DCOPY_PK; -- 사라졌음
 
ALTER TABLE DEPT_COPY
 DROP CONSTRAINT DCOPY_UQ
 MODIFY LNAME NULL; -- 태초의 상태로..
-------------------------------------------------------------------------------- 
...
-- SYS_C007175 LOCATION_ID NN 자기꺼 가져오기
-- SYS_C007175 => DCOPY_LID_NN
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C007174 TO DCOPY_LID_NN;

-- 3_3) 테이블명 변경 : RENAME [기존테이블명] TO 바꿀 테이블명
...
-- 단, 어딘가에서 참조되고 있는 부모테이블 함부로 삭제 안됨!!
-- 만약에 삭제하고자 한다면
-- 방법 1. 자식테이블 먼저 삭제 한 후 부모테이블 삭제하는 방법
-- 방법 2. 부모테이블만 삭제하는데 제약조건까지 같이 삭제하는 방법
--        DROP TABLE 테이블명 CASCADE CONTSTRAINT;
