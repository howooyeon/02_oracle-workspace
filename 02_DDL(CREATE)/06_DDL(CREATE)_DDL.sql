...
WHERE GRADE_CODE = 10;
-- ORA-02292: integrity constraint (DDL.SYS_C007127) violated - child record found
--> 자식테이블 (MEM)에 10 이라는 값을 사용하고 있기 때문에 삭제가 안됨!

DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 30;
--> 자식 테이블 (MEM)에 30이라는 값을 사용하고 있지 않기 때문에 삭제가 잘됨!!

--> 자식테이블에 이미 사용하고 있는 값이 있을 경우
--> 부모테이블로부터 무조건 삭제가 안 되게 하는 "삭제제한" 옵션이 걸려있음!!
...
--------------------------------------------------------------------------------
/*
    자식 테이블 생성 외래키 제약조건 부여할 때 삭제옵션 지정가능
...
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
    ...
    WHERE 1 = 0; -- 무조건 false인 조건 : 구조만을 복사하고자 할 때 쓰이는 구문 (데이터값은 필요 없을 때)
...
-- EMPLOYEE 테이블에 DEPT_CODE에 외래키 제약조건 추가(참조하는 테이블(부모) : DEPPARTMENT(DEPT_ID))
...
INSERT INTO DEPARTMENT VALUES ('S1', '테이스부', 'S1');
