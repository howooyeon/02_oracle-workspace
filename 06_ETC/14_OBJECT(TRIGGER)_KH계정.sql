/*
    ...
    (테이블에 이벤트가 발생했을 때)
    자동으로 매번 실행할 내용 미리 정의해둘 수 있는 객체
...
-- EMPLOYEE 테이블에 새로운 행이 INSERT 될 때마다 자동으로 매세지 출력되는 트리거 정의
...
CREATE TABLE TB_PRODUCT(
    PCODE NUMBER PRIMARY KEY, -- 각 행 식별 -- 상품번호
...
-- 상품번호 중복 안 되게끔 매번 새로운 번호 발생시키는 시퀀스 (SEQ_PCODE)
...
-- 이력번호로 매번 새로운 번호 발생시켜서 돌아갈 수 있게 도와주는 시퀀스 (SEQ_DCODE)
...
UPDATE TB_PRODUCT
SET STOCK = STOCK + 20
WHERE PCODE = 205; -- 잘못하고 2번 눌러서 +40됨..
