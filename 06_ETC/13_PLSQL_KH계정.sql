/*
    < PL / SQL >
    PROCEDURE LANGUAGE EXTENTION TO SAL
    
    오라클 자체에 내장되어 있는 절차적 언어
    SQL 문장내에서 변수의 정의, 조건처리(IF), 반복처리(LOOP, FOR, WHILE)등을 지원하여 SQL의 단점 보완
    다수의 SQL문을 한 번에 실행가능(BLOCK 구조) + 예외처리도 가능
...
-- * 간단하게 화면에 HELLO ORACLE 출력! HELLO WORLD출력했던 것 처럼..
BEGIN
...
-------------------------------실습문제-----------------------------------------------
DECLARE
    -- 레퍼런스 타입변수 (EID, ENAME, DTITLE, NCODE)
...
--------------------------------------------------------------------------------
-- 3) IF 조건식1 THEN 실행내용1 ELSIF 조건식2 THEN 실행내용2 ELSE 실행내용 END IF(IF - ELSE IF - ELSE)
...
BEGIN
    ...
    IF BONUS = 0
        THEN DBMS_OUTPUT.PUT_LINE( SALARY || ENAME || TO_CHAR((12 * SALARY), 'L999,999,999'));
    ELSE
